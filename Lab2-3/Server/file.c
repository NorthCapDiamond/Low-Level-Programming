...
uint64_t try_to_write_in_hole(void* data_ptr, uint64_t size_of_data){
	...
	while(hole_ptr!=INVALID_PTR){
		...
		if(iter_hole->size_of_hole == size_of_data){
			...
			if(iter_hole->size_of_hole > size_of_data){
				...
				if(iter_hole->prev_ptr!=INVALID_PTR){
					struct hole* previous_hole = read_file(iter_hole->prev_ptr, sizeof(struct hole));
				...
				if (iter_hole->next_ptr!=INVALID_PTR){
					struct hole* nextt_hole = read_file(iter_hole->next_ptr, sizeof(struct hole));
				...
				if (file_header->first_hole_ptr==iter_hole->hole_ptr){
					if(iter_hole->next_ptr==INVALID_PTR){
						...
						struct hole* current_hole = read_file(file_header->first_hole_ptr, sizeof(struct hole));
						uint64_t address;
						while(current_hole->size_of_hole > iter_hole->size_of_hole){
							address = current_hole->next_ptr;
							free(current_hole);
							current_hole = read_file(address, sizeof(struct hole));
						}
						...
						if(current_hole->prev_ptr){
							struct hole* before_current_hole = read_file(current_hole->prev_ptr, sizeof(struct hole));
...
uint64_t write_file(void* data_ptr, uint64_t size_of_data){
	...
	uint64_t return_try_hole = try_to_write_in_hole(data_ptr, size_of_data);
...
void delete_from_file(uint64_t offset, uint64_t length){
	...
	struct hole* iter_hole = read_file(file_header->first_hole_ptr, sizeof(struct hole));
	uint64_t address;

	while ((iter_hole->size_of_hole > new_hole.size_of_hole) && iter_hole->next_ptr!=INVALID_PTR) {
		address = iter_hole->next_ptr;
		free(iter_hole);
		iter_hole = read_file(address, sizeof(struct hole));
	...
	if(iter_hole->prev_ptr){
		struct hole* prev_hole = read_file(iter_hole->prev_ptr, sizeof(struct hole));
		prev_hole->next_ptr = write_file(&new_hole, sizeof(struct hole));
		...
		iter_hole->prev_ptr = write_file(&new_hole, sizeof(struct hole));
		fseek(file, iter_hole->prev_ptr, SEEK_SET);
		fwrite(&new_hole, 1, sizeof(struct hole), file);
	

		file_header->first_hole_ptr = write_file(&new_hole, sizeof(struct hole));

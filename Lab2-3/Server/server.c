...
extern Answer* my_answer;


void send_message(Answer msg, int sockfd){
    uint64_t message_size = answer__get_packed_size(&msg);
    printf("Message size is %llu\n", message_size);
    uint8_t* buffer = malloc(message_size);
    answer__pack(&msg, buffer);
...
void server_jobs(int connfd, struct sockaddr_in cli){
    ...
    Answer first_message = ANSWER__INIT;
